...
| `ApngMax` | Multi-image APNG Max policy: retain shared per-stream scheduling while running the full Max route set against the simplified `ApngDefault` floor. Original-source Max receives a positive concurrent share beside deft4j-derived refinement and a selected floor continuation, because the immediate order of complete parents does not prove the order of their search endpoints. |
...
flowchart TD
    ...
    R4 -- "No" --> DEFAULTEND["Retain complete Default result"]
    JOINT --> DEFAULTEND
...
flowchart TD
    ...
    SPAN --> R4
    R4 -- "Yes" --> JOINT["🟡 Choose payload lengths and repeats together"]
    R4 -- "No" --> PICKMAX["Select byte/meaningful-bit winner<br/>or relaxed raw source"]
    JOINT --> PICKMAX
```
