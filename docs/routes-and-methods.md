...
| Length-limited Huffman families | Default tries DeflOpt-heap and Columbo order-heap variants; max adds generic, Columbo/Defluff, exact Defluff, and deft4j Java-heap shapes under a capped cross-product. Generic variant zero uses an exact sorted two-front merge, variant one retains the output-equivalent binary heap, and the two mixed-tie variants retain the original scanning topology. Wrapped-frequency inputs fall back to the heap. | 🔴 | **DeflOpt / Defluff / deft4j / Columbo** as labelled in source; independent **Columbo** hot paths |
