...
| PNG decoded-equivalent frame reuse | After image-job optimization, checksum/size groups are decoded and byte-compared before the best compressed spelling is reused. Retained comparison data is capped at 32 MiB and comparison work at 64 MiB. | 🟡 |
...
| G0t · Max terminal share | Max under `Complete`, `CompleteThenBounded`, or `ApngMax`, with nonzero allowance, source compressed and decoded size each ≤1 MiB and ≤128 source blocks. This is the common Max terminal header work class. R1 restoration retains its 128 KiB gates; R2–R5 retain that smaller envelope in Default and its mandatory comparison work. | `Complete` and `CompleteThenBounded` primary routes receive 4/5 of the original allowance. An `ApngMax` child keeps 19/20 of its assigned image-job slice for primary routes and reserves 1/20 for terminal work. Both primary phases have zero phase grace; terminal tree floors and R1–R12 use the original full deadline and original grace for that stream. A primary yield is not a file timeout. Both phases grow with the allowance; no worker or extra time window is added. Other shared/established container children retain their earlier schedule. M10d's coarse rescue remains limited to policies owning complete stream terminal time. [Validation](research/terminal-closure-validation.md#follow-up-on-13-september-2026-the-larger-max-work-class); [APNG evidence](benchmark-miss-regression-audit.md#apng-terminal-share-follow-up-on-22-september-2026). |
...
this same coarse rescue while the original soft deadline remains open only
under `Complete` or `CompleteThenBounded`; `ApngMax` skips that overrun. It
