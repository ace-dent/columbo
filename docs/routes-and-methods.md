...
flowchart TD
    ...
    POLICY -- "Complete" --> CFLOOR["🟡 Mandatory ordinary comparison floor<br/>optional R6 within remaining Max allowance"]
...
flowchart TD
    ...
    R6 -- "No" --> PICKMAX["Select byte/meaningful-bit winner<br/>or relaxed raw source"]
    ALPHABET --> PICKMAX
```
...
with R6 before continuing its historical Max seed. This optional sibling uses
the owner's existing deadline or assigned route window; it adds no mandatory
Default work, extra time or grace. The final Max incumbent may also receive R6
