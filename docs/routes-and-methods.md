...
| Deflate decode tables | Parser-only canonical tables carry code lengths and payload extra-bit metadata. Literal/length roots use ten bits, distance roots use eight, and the code-length alphabet naturally caps at seven. Direct prefixes are expanded with contiguous copies before subtable pointers are installed. | 🟢 | independent **Columbo** hot path, informed by general decode-table tradeoffs documented by libdeflate |
| Planned bit emission | Final emission reserves the exactly priced output once, enforces that bit limit on every write, and therefore skips repeated capacity reservations. Test and fixture writers without a plan retain ordinary fallible growth. | 🟢 | independent **Columbo** hot path |
