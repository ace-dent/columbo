...
| Exact Zopfli RLE-friendly tree | Max only: preserves useful equal-count runs, averages eligible nearby-count strides, builds one paired literal/distance tree candidate, then prices its complete header and payload against the original frequencies. It remains independent of the existing tree-family cross-product. | 🟡 | **Zopfli** `OptimizeHuffmanForRle` behavior; independently written **Columbo** integration and exact scoring |
